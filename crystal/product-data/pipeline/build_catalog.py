    ...
    for rel_dir in dirs:
        ...
        for root, dirnames, filenames in os.walk(abs_dir):
            ...
            for fn in filenames:
                ...
                # Strip hyphens/underscores (they're internal to a SKU token, e.g.
                # "CL-008") but collapse actual whitespace to a single preserved
                # space, not delete it — dropping whitespace too fused a SKU onto
                # its own trailing angle number ("CPW001 01" -> "CPW00101"),
                # making blob_contains_sku's same-token collision guard misfire
                # and reject what was really an exact match.
                def _blobify(text):
                    return re.sub(r"[^A-Za-z0-9\s]", " ", re.sub(r"[-_]", "", text))
                blob = re.sub(r"\s+", " ", _blobify(stem) + " " + _blobify(folder_name)).strip().upper()
                ...
                own_sku_match = SKU_RE.search(stem) or SKU_RE.search(folder_name)
                candidates.append({
                    "path": fpath,
                    "filename": fn,
                    "stem": stem,
                    ...
                    "own_sku": own_sku_match.group(1).upper() if own_sku_match else None,
                    "width": w,
                    "height": h,
                })
    return candidates
...
def match_products_to_images(products, candidates_by_sheet):
    ...
    for sheet_name, candidates in candidates_by_sheet.items():
        ...
        # Candidates that carry a recognizable SKU-like code of their own are
        # excluded here: tier 1 already tried and failed to match that code
        # against a real product, which means it belongs to a different (often
        # discontinued) SKU — fuzzy-assigning it to an unrelated product by
        # name similarity would silently show the wrong product's photo.
        remaining_products = [p for p in sheet_products if "_images" not in p]
        remaining_candidates = [c for c in candidates if c["path"] not in claimed_paths and not c["own_sku"]]
        by_name_guess = {}
        for c in remaining_candidates:
            by_name_guess.setdefault(c["name_guess"], []).append(c)
        # A single generic category word ("cookware", "cutlery", "fork") is not a
        # real identifying signal — it scores as a near-perfect match against
        # every product in the sheet whose name happens to contain that word,
        # which is how two unrelated products previously ended up sharing photos.
        # Require at least two tokens of real descriptive overlap.
        by_name_guess = {ng: files for ng, files in by_name_guess.items() if len(ng.split()) >= 2}
